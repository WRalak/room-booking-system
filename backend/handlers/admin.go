...
func (h *AdminHandler) GetAllRooms(c *gin.Context) {
	...
	if err := h.db.Order("created_at DESC").Find(&rooms).Error; err != nil {
...
func (h *AdminHandler) ManageRoom(c *gin.Context) {
	...
	if c.Request.Method == "POST" {
		...
		if err := h.db.First(&existingRoom, roomID).Error; err != nil {
		...
		if err := h.db.Model(&existingRoom).Updates(updates).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update room"})
			return
		}

		h.db.First(&existingRoom, roomID)
...
func (h *AdminHandler) DeleteRoom(c *gin.Context) {
	...
	if err := h.db.Delete(&models.Room{}, roomID).Error; err != nil {
