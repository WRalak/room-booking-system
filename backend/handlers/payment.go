...
func (h *PaymentHandler) InitializePaystackPayment(c *gin.Context) {
	...
	reference := paystackReq["reference"].(string)
...
func (h *PaymentHandler) InitiateMpesaPayment(c *gin.Context) {
	...
	// Format phone number to international Kenya format (2547XXXXXXXX)
...
func (h *PaymentHandler) GetPaymentStatus(c *gin.Context) {
	...
	var payment models.Payment
	if err := h.db.Where("reference = ? OR provider_ref = ?", reference, reference).First(&payment).Error; err != nil {
...
func (h *PaymentHandler) generateMpesaPassword(timestamp string) string {
	raw := h.config.(*AppConfig).MpesaShortcode + h.config.(*AppConfig).MpesaPasskey + timestamp
